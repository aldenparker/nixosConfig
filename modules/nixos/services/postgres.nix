...
in {
  ...
  options.snowman.${module-category}.${module-name} = {
    enable = mkEnableOption "Enables ${module-name} and sets database path for host. Use ensureDatabases and ensureUsers in other modules to create databse for that module.";
  ...
  config = mkIf cfg.enable {
    ...
    systemd.tmpfiles.rules = [
      "d ${cfg.dataPath} 0755 postgres postgres"
    ];
