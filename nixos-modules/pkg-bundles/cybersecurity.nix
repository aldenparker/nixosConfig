...
with lib;
let
  cyberchef-vSNOW = flake-inputs.self.packages.${system}.cyberchef; # Fixed cyberchef
  cfg = config.${namespace}.pkg-bundles.cybersecurity; # Config path
in
{
  # --- Set options
  options.${namespace}.pkg-bundles.cybersecurity = {
    enable = mkEnableOption "Installs cybersecurity / pentesting packages and makes nessesary configurations.";
  };
  ...
  config = mkIf cfg.enable {
    environment.systemPackages = with pkgs; [
      ...
      # ike-scan # Tool to discover, fingerprint and test IPsec VPN servers TODO: Broken
      ...
      cyberchef-vSNOW # Common decoding tool
    ];
